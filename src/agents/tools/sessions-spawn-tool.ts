  ...
  const schema = {
    ...
    allowedTools: Type.Optional(
      Type.Array(Type.String(), {
        description:
          "Allowlist of tool names the subagent may use. Mutually exclusive with deniedTools.",
      }),
    ),
    deniedTools: Type.Optional(
      Type.Array(Type.String(), {
        description:
          "Denylist of tool names the subagent may not use. Mutually exclusive with allowedTools.",
      }),
    ),
    allowedNodes: Type.Optional(
      Type.Array(Type.String(), {
        description:
          "Allowlist of node names the subagent may access. Mutually exclusive with deniedNodes.",
      }),
    ),
    deniedNodes: Type.Optional(
      Type.Array(Type.String(), {
        description:
          "Denylist of node names the subagent may not access. Mutually exclusive with allowedNodes.",
      }),
    ),
  };
      ...
      const result = await spawnSubagentDirect(
        {
          ...
          toolConstraints: (() => {
            ...
            const deniedTools = Array.isArray(params.deniedTools)
              ? (params.deniedTools as string[])
              : undefined;
            const allowedNodes = Array.isArray(params.allowedNodes)
              ? (params.allowedNodes as string[])
              : undefined;
            const deniedNodes = Array.isArray(params.deniedNodes)
              ? (params.deniedNodes as string[])
              : undefined;
            if (!browserProfile && !allowedTools && !deniedTools && !allowedNodes && !deniedNodes) {
              return undefined;
            }
            return {
