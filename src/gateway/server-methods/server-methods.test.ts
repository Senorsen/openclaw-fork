...
describe("injectTimestamp", () => {
  ...
  it("prepends a compact timestamp matching formatZonedTimestamp", () => {
    ...
    expect(result).toMatch(/^\[周三 2026-01-28 20:30:00 EST\] Is it the weekend\?$/);
  });

  it("uses channel envelope format with DOW prefix", () => {
    const now = new Date();
    const expected = formatZonedTimestamp(now, { timeZone: "America/New_York", displaySeconds: true });

    const result = injectTimestamp("hello", { timezone: "America/New_York" });

    expect(result).toBe(`[周三 ${expected}] hello`);
  });
  ...
  it("uses the configured timezone", () => {
    ...
    expect(result).toMatch(/^\[周三 2026-01-28 19:30:00 CST\]/);
  });

  it("defaults to UTC when no timezone specified", () => {
    const result = injectTimestamp("hello", {});

    expect(result).toMatch(/^\[周四 2026-01-29 01:30/);
  });
  ...
  it("does NOT double-stamp messages already injected by us", () => {
    const alreadyStamped = "[周三 2026-01-28 20:30:00 EST] hello there";
  ...
  it("handles midnight correctly", () => {
    ...
    expect(result).toMatch(/^\[周日 2026-02-01 00:00:00 EST\]/);
  });
  ...
  it("handles date boundaries (just before midnight)", () => {
    ...
    expect(result).toMatch(/^\[周六 2026-01-31 23:59:00 EST\]/);
  });
  ...
  it("accepts a custom now date", () => {
    ...
    expect(result).toMatch(/^\[周五 2025-07-04 12:00:00 EDT\]/);
  });
});
