...
describe("formatAgentEnvelope", () => {
  it("includes channel, from, ip, host, and timestamp", () => {
    withEnv({ TZ: "UTC" }, () => {
      ...
      expect(body).toBe("[WebChat user1 mac-mini 10.0.0.5 周四 2025-01-02T03:04:00Z] hello");
    });
  });
  ...
  it("formats timestamps in UTC when configured", () => {
    withEnv({ TZ: "America/Los_Angeles" }, () => {
      ...
      expect(body).toBe("[WebChat 周四 2025-01-02T03:04:00Z] hello");
    });
  });
  ...
  it("formats timestamps in user timezone when configured", () => {
    ...
    expect(body).toMatch(/\[WebChat 周四 2025-01-02 04:04:00 [^\]]+\] hello/);
  });
